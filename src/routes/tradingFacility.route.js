...
router.post('/', auth, checkPermission(['users', 'admin', 'trading_facility.create']), ctrl.create);

// PUT  /api/trading-facilities/:id     — cập nhật (cần đăng nhập + quyền)
router.put('/:id', auth, checkPermission(['users', 'admin', 'trading_facility.update']), ctrl.update);

// DELETE /api/trading-facilities/:id   — xóa (cần đăng nhập + quyền)
router.delete('/:id', auth, checkPermission(['users', 'admin', 'trading_facility.delete']), ctrl.remove);
